...
/*   Updated: 2019/11/23 06:28:57 by trobicho         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	Mesh::add_needed_vertex(s_vertex v, s_vec3i l, std::bitset<8> &v_b
			, uint32_t v_idx[8])
{
	s_vertex	vc = v;
...
void	Mesh::add_cube_from_node(s_vec3i v, e_block_type type, void *node_ptr)
{
	...
	if (node->get_child_slog().x == 1)
		add_needed_vertex(vertex, node->get_child_slog(), v_b, v_idx);
	else
		add_needed_vertex(vertex, node->get_slog(), v_b, v_idx); // weird
	if (!m_moore_access[MOORE_UP])
	{
		add_index(v_idx[2]);
		add_index(v_idx[1]);
